  ...
  const preprocess = (str) => {
    ...
    if (KRONOS_VARIANTS.some((x) => new RegExp(x, "gmi"))) {
      for (const variant of KRONOS_VARIANTS) {
        for (const varCase of [variant.toLowerCase(), variant.toUpperCase()]) {
          out = out.replace(varCase, `<span class="redacted">REDACTED</span>`);
        }
      }
    }
...
async function touchOfTheWarp(delayMs) {
  ...
  for (const node of nodes) {
    if (
      !(
        ["aquila", "archives-text"].some((x) => node.classList.contains(x)) ||
        ["footer", "aquila-container", "pageTitle", "header"].includes(node.id)
      )
    ) {
